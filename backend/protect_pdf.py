from restrictions import check_restrictions, get_user, check_protect_pdf_restrictions
...
def protect_pdf():
    ...
    try:
        ...
        # Run general restrictions check
        restriction = check_restrictions(email, [temp_path])
        if restriction:
            return jsonify({"error": restriction}), 403
            
        # Run premium-only feature check
        premium_restriction = check_protect_pdf_restrictions(email, [temp_path])
