...
def convert_pptx_to_pdf():
    ...
    try:
        ...
        if not filename.lower().endswith(('.ppt', '.pptx')):
            return jsonify({"error": f"File {filename} is not a PowerPoint presentation"}), 400
        ...
        # Convert PowerPoint to PDF using COM (Windows only) if comtypes is available
        if platform.system() == 'Windows' and comtypes is not None:
            try:
                powerpoint = comtypes.client.CreateObject('Powerpoint.Application')
                powerpoint.Visible = True  # Sometimes needed for PowerPoint
        ...
        print("?? ERROR in convert_pptx_to_pdf function:")
        traceback.print_exc()
        return jsonify({"error": str(e)}), 500

def _convert_with_libreoffice(input_path, output_path):
    """
    Convert PowerPoint to PDF using LibreOffice/OpenOffice
    """
    try:
