...
def convert_jpg_to_pdf():
    ...
    try:
        ...
        # Check if all files are JPG/JPEG
        for file in files:
            filename = file.filename
            if not filename.lower().endswith(('.jpg', '.jpeg')):
                return jsonify({"error": f"File {filename} is not a JPG/JPEG image"}), 400
        ...
        print(f"[DEBUG] /convert-jpg-to-pdf email={email}")
        restriction_error = check_convert_pdf_restrictions(email, temp_file_paths)
        if restriction_error:
            print(f"[DEBUG] /convert-jpg-to-pdf restriction: {restriction_error}")
        ...
        print("🔥 ERROR in convert_jpg_to_pdf function:")
        traceback.print_exc()
